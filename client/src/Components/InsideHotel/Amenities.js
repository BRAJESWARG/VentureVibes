import React from "react";
import "./InsideHotel.css";
import "react-responsive-carousel/lib/styles/carousel.min.css";



const Amenities = () => {

    return (
        <div className="Amenities">
            <div id="amenities" className="Amenitiesstyles__AmenitiesOuterWrapper-sc-e2msby-2 jKkVic">
                <div className="Amenitiesstyles__AmenitiesWrapper-sc-e2msby-3 cdZIRD">
                    <h2 className="dwebCommonstyles__LargeSectionHeader-sc-112ty3f-8 Amenitiesstyles__AmenitiesHeaderStyled-sc-e2msby-4 eeTyKB fRCeTy">Amenities at Shakespeare Corporate Guest House</h2>
                    <div className="Amenitiesstyles__AmenitiesListBlock-sc-e2msby-5 jtaVPN">
                        <span className="Amenitiesstyles__PopularAmenitiesTag-sc-e2msby-7 kOWhTi">
                            <h3 className="Amenitiesstyles__PopularAmenitiesTagTextStyled-sc-e2msby-8 kCaNwu">POPULAR AMENITIES</h3>
                        </span>
                        <ul className="Amenitiesstyles__AmentiesHorizontalListof4-sc-e2msby-9 ixObVF">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="2.4rem" height="2.4rem" className="HouseKeepingIcon-sc-5ioupd-0 iYVwtt">
                                <path d="M13.261 24.744a3.2 3.2 0 1 1 0 6.4 3.2 3.2 0 0 1 0-6.4m5.12-24.32a8.32 8.32 0 0 1 8.32 8.32v16.64c0 .353.287.64.64.64h2.346a1.28 1.28 0 0 1 1.215.876l.852 2.56a1.28 1.28 0 0 1-1.214 1.685H20.3a1.28 1.28 0 0 1-1.214-1.686l.854-2.56.055-.138a1.28 1.28 0 0 1 1.16-.736h2.346a.64.64 0 0 0 .64-.64V8.745a5.76 5.76 0 0 0-11.52 0c0 .353.287.64.64.64.707 0 1.28.573 1.28 1.28v5.126a.64.64 0 0 1-.64.64H3.237a.64.64 0 0 1-.555-.957 7 7 0 0 1 .445-.694 6.56 6.56 0 0 1 4.458-2.765.64.64 0 0 0 .556-.64v-.712c0-.707.573-1.28 1.28-1.28a.64.64 0 0 0 .64-.64 8.32 8.32 0 0 1 8.32-8.32zm-4.48 17.92a.64.64 0 0 1 .64.64v3.237a.64.64 0 0 1-.723.64 5 5 0 0 0-.557-.037 5.12 5.12 0 0 0-5.12 5.12 5 5 0 0 0 .23 1.507.32.32 0 0 1-.305.413H4.035a1.92 1.92 0 0 1-1.828-1.332 19.3 19.3 0 0 1-.778-9.676.64.64 0 0 1 .626-.512z">
                                </path>
                            </svg>
                            <span className="Amenitiesstyles__AmenityItemText-sc-e2msby-10 hkBexV">Caretaker</span>
                            <svg viewBox="0 0 16 16" width="2.4rem" height="2.4rem" className="RoundedCheckbox-sc-4skzta-0 chqHwc">
                                <g fill="none" fill-rule="evenodd">
                                    <path d="M-4-4h24v24H-4z">
                                    </path>
                                    <path fill="#898B91" d="M7.68 0a7.68 7.68 0 1 1 0 15.36A7.68 7.68 0 0 1 7.68 0m4.335 3.922-6.233 6.702-2.454-2.26a.436.436 0 0 0-.622.021.45.45 0 0 0 .022.632l2.556 2.358c.295.28.76.26 1.039-.032l6.326-6.8a.45.45 0 0 0-.011-.632.436.436 0 0 0-.623.011">
                                    </path>
                                </g>
                            </svg>
                            <span className="Amenitiesstyles__AmenityItemText-sc-e2msby-10 hkBexV">Housekeeping</span>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 15" width="2.4rem" height="2.4rem" className="HappyWifiIcon-sc-9vspjr-0 hFyVkv">
                                <path d="M8 11.295a1.794 1.794 0 1 1 0 3.588 1.794 1.794 0 0 1 0-3.588M4.322 8.693A5.2 5.2 0 0 1 9.71 7.465c.74.254 1.411.673 1.965 1.225a.897.897 0 0 1-.634 1.531l-.117-.007a.9.9 0 0 1-.515-.254l-.067-.055-.069-.056a3.41 3.41 0 0 0-4.683.111.9.9 0 0 1-.634.264l-.12-.008a.898.898 0 0 1-.514-1.523M2.293 5.947a8.08 8.08 0 0 1 11.412 0 .9.9 0 0 1 .262.633v.021a.89.89 0 0 1-.893.876l-.118-.008a.9.9 0 0 1-.516-.254 6.286 6.286 0 0 0-8.878 0 .898.898 0 0 1-1.268-1.268zM.263 3.2c4.276-4.267 11.198-4.267 15.474 0a.894.894 0 0 1-.402 1.5.9.9 0 0 1-.866-.232 9.16 9.16 0 0 0-12.939 0A.897.897 0 0 1 .262 3.2z">
                                </path>
                            </svg>
                            <span className="Amenitiesstyles__AmenityItemText-sc-e2msby-10 hkBexV">Free Wi-Fi</span>
                            <svg viewBox="0 0 16 16" width="2.4rem" height="2.4rem" className="RoundedCheckbox-sc-4skzta-0 chqHwc">
                                <g fill="none" fill-rule="evenodd">
                                    <path d="M-4-4h24v24H-4z">
                                    </path>
                                    <path fill="#898B91" d="M7.68 0a7.68 7.68 0 1 1 0 15.36A7.68 7.68 0 0 1 7.68 0m4.335 3.922-6.233 6.702-2.454-2.26a.436.436 0 0 0-.622.021.45.45 0 0 0 .022.632l2.556 2.358c.295.28.76.26 1.039-.032l6.326-6.8a.45.45 0 0 0-.011-.632.436.436 0 0 0-.623.011">
                                    </path>
                                </g>
                            </svg>
                            <span className="Amenitiesstyles__AmenityItemText-sc-e2msby-10 hkBexV">Living Room</span>
                        </ul>
                    </div>
                    <div className="Amenitiesstyles__CategorizedAmenitiesTableContainer-sc-e2msby-11 gpcTKG">
                        <div id="Popular Amenities" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">Popular Amenities</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Caretaker</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Housekeeping</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Free Wi-Fi</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Living Room</li>
                            </ul>
                        </div>
                        <div id="Basic Facilities" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">Basic Facilities</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Housekeeping</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Air Conditioning</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Telephone</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Bathroom</li>
                            </ul>
                        </div>
                        <div id="Food and Drinks" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">Food and Drinks</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Dining Area</li>
                            </ul>
                        </div>
                        <div id="Safety and Security" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">Safety and Security</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">CCTV</li>
                            </ul>
                        </div>
                        <div id="General Services" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">General Services</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Caretaker</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Luggage Storage</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Electrical Sockets</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Doctor on Call</li>
                            </ul>
                        </div>
                    </div>
                    {/* <div className="Amenitiesstyles__CategorizedAmenitiesTableContainer-sc-e2msby-11 fEmIHs">
                        <div id="Popular Amenities" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">Popular Amenities</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Caretaker</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Housekeeping</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Free Wi-Fi</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Living Room</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Dining Area</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Power Backup</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Luggage Storage</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Seating Area</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Balcony/Terrace</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Air Conditioning</li>
                            </ul>
                        </div>
                        <div id="Basic Facilities" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">Basic Facilities</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Housekeeping</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Air Conditioning</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Telephone</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Bathroom</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Free Wi-Fi</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Smoking Rooms</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Umbrellas</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Refrigerator</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Room Service</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Power Backup</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Torch</li>
                            </ul>
                        </div>
                        <div id="Food and Drinks" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">Food and Drinks</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Dining Area</li>
                            </ul>
                        </div>
                        <div id="Safety and Security" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">Safety and Security</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">CCTV</li>
                            </ul>
                        </div>
                        <div id="General Services" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">General Services</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Caretaker</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Luggage Storage</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Electrical Sockets</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Doctor on Call</li>
                            </ul>
                        </div>
                        <div id="Common Area" className="Amenitiesstyles__OneCateoryOfAmenitiesContainer-sc-e2msby-12 hiqBoU">
                            <h3 className="Amenitiesstyles__HeadingOneCategoryOfAmenitiesBlockStyled-sc-e2msby-13 dlgwqi">Common Area</h3>
                            <ul className="Amenitiesstyles__OneCategoryOfAmenitiesList-sc-e2msby-14 cQHPky">
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Living Room</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Seating Area</li>
                                <li className="Amenitiesstyles__OneCategoryOfAmenitiesListItem-sc-e2msby-15 jDhMTW">Balcony/Terrace</li>
                            </ul>
                        </div>
                    </div> */}
                    {/* <div className="Amenitiesstyles__ShowHideLink-sc-e2msby-6 kGlxow">
                        <a href="/" className="dwebCommonstyles__PrimaryLink-sc-112ty3f-11 bmajuy">Show All Amenities</a>
                    </div>
                    <div className="Amenitiesstyles__ShowHideLink-sc-e2msby-6 kGlxow">
                        <a href="/" className="dwebCommonstyles__PrimaryLink-sc-112ty3f-11 bmajuy">Hide</a>
                    </div> */}
                </div>
            </div>
        </div>
    );
}

export default Amenities;